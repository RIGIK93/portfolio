<script lang="ts">
    ...
    export let themeOptions: [string, string];

    let checked: boolean;

    function checked_to_theme() {
        return checked ? themeOptions[0] : themeOptions[1]
    }

    function theme_to_checked(theme: string) {
        return theme == themeOptions[1] ? false : true
    }

    function storage_get() {
        let item = localStorage.getItem("theme")
        if (item == null || !themeOptions.includes(item)) {
    ...
    function storage_set() {
        return localStorage.setItem("theme", checked_to_theme())
    }

    // $: document.querySelector('html')!.setAttribute("data-theme", to_value())

    function updateCheckbox() {
        document.querySelector('html')!.setAttribute("data-theme", checked_to_theme())
        storage_set()
    }

    onMount(() => {
        // checked = theme_to_checked(document.querySelector('html')!.getAttribute('data-theme')!) 
        checked = theme_to_checked(storage_get()) 
        updateCheckbox()
    })
</script>
...
<label class="swap swap-rotate" aria-label="Theme Switcher">
    ...
    <input type="checkbox" class="theme-controller" bind:checked={checked} on:change={updateCheckbox} value="synthwave" aria-label="Switch Theme" />
